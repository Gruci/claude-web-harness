...
| `session_id` 식별 불가 | 태그 없는 행만 차단 | 남의 행은 지울 권한이 없어 전 행 차단은 종료 불능이다 |
...
| 42 | 해시 네비 단일 기전 | `hash_nav` | 이벤트를 안 내는 History API 혼용 — 주소만 바뀌고 화면이 안 따라온다 |
...
| 48 | 아키텍처 그림 1:1 대조 | `arch_diagram` · `arch_diagram_engine:<파일>` | 코드 어디인지 증명 안 된 노드, 이름 바꾸고 안 고친 그림, 렌더 안 한 정본. 엔진 위임분은 node 없으면 `[TOOL]` — 통과가 아니다. 정본은 `dev/DIAGRAM.md` |
