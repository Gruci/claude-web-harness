...
검사 대상 확장자는 프로파일의 `SOURCE_EXT`·`UI_EXT` 가 정한다. 32종 중 **8종은 파이썬 구문
분석·관용구에 묶여 있고**(2·8·9·11·12·13·14·16), `SYNTAX` 가 `"python"` 이 아니면 실행하지
않고 `[SKIP]` 으로 사유를 남긴다.

실행을 막는 이유가 둘이다. `ast.parse` 를 다른 언어에 돌리면 전 파일이 "파싱 실패" 위반이
되고, `os.getenv` 같은 파이썬 관용구 정규식은 Go 의 `os.Getenv` 에 안 걸려 `[OK]` 로
통과한다. **후자가 더 위험하다** — 검사기가 아무것도 안 보면서 초록불을 내는 상태다.

Go 픽스처(`tests/fixtures/goproj`)가 이 동작을 동결한다: 7종 실행 · 8종 언어로 `[SKIP]` ·
15종 설정 미비로 `[SKIP]`.
