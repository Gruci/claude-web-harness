...
> 담는 것: 리스된 manual Apply 이벤트 1건을 실제 소스에 반영하는 규칙과 반환 JSON 계약. 담지 않는 것: 폴링과 프로토콜 응답(부모 live 스레드가 소유)·Impeccable 디자인 지침(→ `.claude/skills/impeccable/SKILL.md`). 읽는 시점: live 매뉴얼 편집 배치를 적용하도록 위임받을 때.
