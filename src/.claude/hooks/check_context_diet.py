...
배경(2026-07-20 실사고): 95KB 워크플로우 결과·60KB 다이제스트를 메인 루프가 통으로 Read →
그 뒤 모든 턴에서 재전송·재과금. "요약만 받는다"는 산문 규칙은 드리프트하므로 훅으로 강제한다.

2026-07-28 Opus 5 전환 — 바이트 상한을 토큰 추정으로 교체. 한글은 UTF-8 3바이트/자라
바이트가 토큰 비용의 프록시로 부정확했다(423줄 정본 MD가 62KB로 걸려, 라우팅표가 "읽어라"고
지시하는 필독 문서를 훅이 막는 자기모순). 메인 루프 1M 컨텍스트 기준으로 임계도 상향.

판정 (메인/서브 구분 없이 동작하는 설계 — 훅은 서브에이전트 툴 호출에도 발화):
  - 텍스트 파일 추정 토큰이 LIMIT_TOKENS 초과 && 분할 파라미터(limit ≤ CHUNK_LIMIT_LINES) 없음 → exit 2 차단.
  - 이미지/바이너리는 예외(비전 에이전트가 통으로 읽어야 함 — 기계 판별 불가, 산문 규칙 유지).
...
def _estimate_tokens(path: Path) -> int:
    """ASCII ≈ 4자/토큰, 비ASCII(한글 등) ≈ 1자/토큰 — ±20% 근사면 차단 판정에 충분."""
    try:
...
def main() -> None:
    ...
    print(
        f"[CONTEXT GUARD] {path.name} ≈ {estimated:,}토큰 — 통읽기 차단 (상한 {LIMIT_TOKENS:,}).\n"
        f"  · offset/limit(≤{CHUNK_LIMIT_LINES}줄)으로 필요한 부분만 나눠 읽어라.\n"
