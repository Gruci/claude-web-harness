...
# ── 언어 ───────────────────────────────────────────────────────────────────────
# 게이트가 볼 파일 확장자. 서버 언어가 파이썬이 아니면 여기를 갈아끼운다.
SOURCE_EXT: tuple[str, ...] = ("*.py",)          # 예: ("*.go",) · ("*.rs",) · ("*.rb",)
UI_EXT: tuple[str, ...] = ("*.tsx", "*.ts")      # 화면 소스. 예: ("*.vue", "*.ts")

# 서버 언어 이름. "python" 일 때만 구문 분석·파이썬 관용구에 의존하는 검사 8종이 돈다
# (중첩 def·타입힌트·Any·커넥션 블록·환경변수·await 없는 async·import 경로·라우트 응답).
# 다른 값이면 그 8종은 [OK] 가 아니라 [SKIP] 으로 사유와 함께 찍힌다 — 파이썬 정규식이
# 다른 언어에서 안 걸리는 것을 "위반 없음"으로 보고하면 그게 무음 통과다.
SYNTAX: str | None = "python"
