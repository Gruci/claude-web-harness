...
**서버 언어가 파이썬이 아니면 `SOURCE_EXT` 와 `SYNTAX` 부터 고친다.** 프리셋은 전부
파이썬 기준이라 확장자가 안 맞으면 대상이 0건이고, 그 상태에서는 나머지를 아무리 채워도
안 돈다. `SYNTAX` 에 실제 언어 이름(`"go"`·`"rust"` 등)을 적으면 파이썬 구문·관용구에
묶인 검사 8종이 `[OK]` 가 아니라 사유와 함께 `[SKIP]` 으로 빠진다 — 파이썬 정규식이
다른 언어에서 안 걸리는 것을 "위반 없음"으로 보고하는 것을 막는다.
